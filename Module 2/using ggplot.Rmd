...
USE geom_jitter() to make ur point more distinct and seperate.

```{r}
ggplot(state_data, aes(x=Date, y=Positive)) # + geom_point()
ggplot(state_data, aes(x= Date, y=Positive)) + geom_point(col = 'red') + geom_jitter()
```
...
Histogram plots!!
```{r}
ggplot(data = state_data, mapping = aes(x= Positive)) + geom_histogram(bins = 5) # here is the data break it into 10 bins of equal sizes

ggplot(data = state_data, mapping = aes(x= Positive)) + geom_histogram(bins = 5, color = "white") #border
...
```{r}
## To plot multiple box plots in a single graph

manystates_data = filter(covid_data, State == c('Delhi', 'Karnataka', 'Tamil Nadu', 'Uttar Pradesh'))
ggplot(manystates_data, mapping = aes(x= factor(State), y= Negative)) + geom_boxplot()
```
...
ggplot(data = manystates_data, mapping = aes(x=State, y = Positive)) + geom_bar()
```
#IMPORTANT 

## Facets!! --> similar to subplots in python
...
ggsave('4 state.png', width = 8, height = 6)
```
...
# here color is categorical
# check for which type of data is each variable --> nominal, ordinal, continuous, etc.

# here all are continuous but check?
```
...
ggplot(mpg, aes(x = displ, y = hwy, color = class)) + geom_point() + 
  labs(title = "Fuel Efficiency by Engine Power",
       ...
       x = "Engine power (litres displacement)",
       y = "Fuel Efficiency (miles per gallon)",
       color = "Car Type") + theme_classic()   # check for more themes
```
