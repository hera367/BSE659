...
```
Advantages of tidyverse: convert your data into tibble
...
Or you can select a few contiguous columns using `:` operator:
...
newdata <- mutate(covid_data, prop = Positive/TotalSamples) # adding a new column to the table
newdata
```
...
```{r}
write.csv(covid_data, file= "covid_data_copy_hera.csv")
```

You may then later want to read the file into your workspace. For this you can use the read command.

```{r}
covid_data <- read.csv(file= "covid_data_copy_hera.csv")
```
